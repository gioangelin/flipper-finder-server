...
// fallback image se nessuna immagine disponibile
const FALLBACK_IMG = "data:image/svg+xml;charset=UTF-8," + encodeURIComponent(
  `<svg xmlns='http://www.w3.org/2000/svg' width='400' height='300'><rect width='100%' height='100%' fill='#f0f0f0'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='#888' font-size='18'>No Image</text></svg>`
);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- Ottieni token eBay (cache in memoria globale) ---
    ...
    const items = (ebayJson.itemSummaries || []).map((item) => {
      ...
      const priceValue =
        ...
        (item.priceRange?.min?.value) ||
        "N/D";

      const currency =
        item.price?.currency ||
        item.currentBidPrice?.currency ||
        (item.priceRange?.min?.currency) ||
        "EUR";
